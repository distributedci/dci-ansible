...
server."""
    ...
    def __init__(self):
        ...
        if self._got_env:
            with open("/tmp/dci_job_id.%s.log" % os.getpid(), "w") as log:
    ...
    def create_file(self, name, content):
        ...
        if self._job_id is None:
            ...
            if self._got_env:
                with open("/tmp/dci_job_id.%s.log" % os.getpid(), "a") as log:
                    log.write("name=%s status=%d json=%s\n" % (name, ret.status_code, ret.json()))
            if ret.status_code // 100 != 2:
                self._file_backlog.append(kwargs)

...
class CallbackModule(CM_default):
    ...
    def _build_dci_context(self):
        ...
        if login is not None and password is not None:
            return dci_context.build_dci_context(url, login, password, user_agent)
        elif client_id is not None and api_secret is not None:
            return dci_context.build_signature_context(
                url, client_id, api_secret, user_agent
            )
    ...
    def banner(self, msg):
        ...
        if self._name:
            ...
            self.create_file(
                prefix + self._name, self._content if self._content != "" else " "
            )
            self._content = ""
    ...
    def create_file(self, name, content):
        def _content_to_utf8():
            try:
                ...
                return (
                    "warn/%s" % name,
                    "invalid content, not able to encode to utf-8: %s" % str(ve),
                )
        ...
        if self._job_id is None:
            ...
            for idx in range(len(self._file_backlog)):
                ret = dci_file.create(self._dci_context, **self._file_backlog[idx])
    ...
    def v2_runner_on_ok(self, result, **kwargs):
        ...
        # variable anymore).
        if "jobstate" in result._result and "id" in result._result["jobstate"]:
        ...
        if (
            result._task.action == "dci_job"
            and "invocation" in result._result
            and (
                result._result["invocation"]["module_args"]["upgrade"]
                or result._result["invocation"]["module_args"]["update"]
            )
        ):
            self._job_id = result._result["job"]["id"]
            self.create_jobstate(
                comment="starting the update/upgrade", status="pre-run"
            )
        # Capture job_id from a set_fact task (needed for dci-pipeline)
        elif (
            result._task.action == "set_fact"
            and "ansible_facts" in result._result
            and "job_id" in result._result["ansible_facts"]
            and result._result["ansible_facts"]["job_id"] is not None
        ):
        ...
        # Capture job_id from a dci_job task (job created in playbook)
            ...
            result._task.action == "dci_job"
            and "job" in result._result
            and "id" in result._result["job"]
            and result._result["job"]["id"] is not None
        ):

            self._job_id = result._result["job"]["id"]
            self.process_backlog()
        # Get job id from environment
        elif self._job_id == None and "DCI_JOB_ID" in os.environ:
    ...
    def process_backlog(self):
        ...
        for rec in self._backlog:
            if "status" in rec:
                ...
                self.create_file(rec["name"], rec["content"])
        self._backlog = []
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        self.create_jobstate(comment=comment, status=play.get_vars().get("dci_status"))
        ...
        if name == "include_tasks":
            if hasattr(result._task, "get_ds"):
                if "include_tasks" in result._task.get_ds():
                    name = "%s: %s" % (
                        name,
                        result._task.get_ds()["include_tasks"],
                    )  # noqa
        return name

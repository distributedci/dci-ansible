...
class CallbackModule(CallbackBase):
    ...
    def _get_var(self, task, var_name):
        """Return a variable value for the current play and task"""
        vm = self._play.get_variable_manager()
        vars_context = vm.get_vars(loader=self._loader, play=self._play, host=None, task=task)
        templar = Templar(loader=self._loader, variables=vars_context)
        return templar.template('{{ %s }}' % var_name, fail_on_undefined=False)
    ...
    def _start_task(self, task):
        ...
        if self._generate_task_name and name.find(self._generate_task_name) != -1:
            log_dir = self._get_var(task, 'job_logs.path')

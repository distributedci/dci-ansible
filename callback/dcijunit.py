...
class CallbackModule(CallbackBase):
    ...
    def _get_var(self, task, var_name):
        ...
        vars_context = vm.get_vars()
        log = open('/tmp/dcijunit.log', 'a')
        log.write('vars_context=%s\n' % vars_context)
        log.close()
        return vars_context.get(var_name, None)
    ...
    def _start_task(self, task):
        ...
        if self._generate_task_name and name.find(self._generate_task_name) != -1:
            log_dir = self._get_var(task, 'job_logs')
            log.write('GENERATED in %s\n' % log_dir.get('path'))
            self._generate_report(log_dir.get('path'))

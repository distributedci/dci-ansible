...
class CallbackModule(CallbackBase):
    ...
    def _all_vars(self, task, host):
        """
        Return all variables for the current play and task.
        Since v2_playbook_on_task_start is called before any host-specific data
        is available, use host=None.
        """
        return self.vm.get_vars(loader=self.loader, play=self.play, host=host, task=task)
    ...
    def _start_task(self, task):
        ...
        if self._generate_task_name and name.find(self._generate_task_name) != -1:
            ...
            vars_context = self._all_vars(task, None)
